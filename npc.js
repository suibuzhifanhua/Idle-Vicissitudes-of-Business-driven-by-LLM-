...
window.UI = (() => {
  ...
  let _stockNeedsFullRefresh = false;

  // ========== StateManager 订阅：G 变化时自动标记仪表板脏 ==========
  (function _setupDashboardSubscriptions() {
    if (typeof StateManager === 'undefined') return;
    var dashboardKeys = ['money', 'reputation', 'stress', 'rank', 'connections', 'employees', 'businesses', 'rivals'];
    var onChange = function() { markDirty('dashboard'); };
    for (var i = 0; i < dashboardKeys.length; i++) {
      StateManager.subscribe(dashboardKeys[i], onChange);
    }
  })();
  ...
  // 属性值颜色映射（≥70绿 / ≥40金 / <40灰）
  function getAttrColor(val) { return val >= 70 ? 'var(--green)' : val >= 40 ? 'var(--accent)' : 'var(--text-muted)'; }

  /**
   * 计算所有已解锁城市中的总业务等级（跨城市汇总）
   * @returns {number} 总业务等级
   */
  ...
  // ========== DOM 增量 Patch 辅助函数 ==========
  // 基于 data-{prefix} 属性追踪 DOM 节点，仅更新变化部分
  function _patchList(container, newHtml, prefix) {
    var temp = document.createElement('div');
    temp.innerHTML = newHtml;
    var newMap = {};
    var newOrder = [];
    var attrName = 'data-' + prefix;
    var nodes = temp.querySelectorAll('[' + attrName + ']');
    for (var i = 0; i < nodes.length; i++) {
      var id = nodes[i].getAttribute(attrName);
      newMap[id] = nodes[i].outerHTML;
      newOrder.push(id);
    }
    // 构建现有 DOM 映射
    var existing = {};
    var existingNodes = container.querySelectorAll('[' + attrName + ']');
    for (var j = 0; j < existingNodes.length; j++) {
      var eid = existingNodes[j].getAttribute(attrName);
      existing[eid] = existingNodes[j];
    }
    // Diff: 删除不在新列表中的节点
    for (var eid2 in existing) {
      if (!(eid2 in newMap)) {
        existing[eid2].remove();
      }
    }
    // 按新顺序重建
    ...
    for (var k = 0; k < newOrder.length; k++) {
      var nid = newOrder[k];
      var newNodeHtml = newMap[nid];
      if (nid in existing) {
        // 已有节点：比较内容，不同则原地替换
        if (existing[nid].outerHTML !== newNodeHtml) {
          existing[nid].outerHTML = newNodeHtml;
        }
        // 保持顺序：如果不是下一个，移动到正确位置
        var expectedNext = container.children[k];
        if (expectedNext !== existing[nid]) {
          container.insertBefore(existing[nid], expectedNext);
        }
      } else {
        // 新节点：从 HTML 创建并插入
        var wrapper = document.createElement('div');
        wrapper.innerHTML = newNodeHtml;
        var newNode = wrapper.firstChild;
        container.insertBefore(newNode, container.children[k] || null);
      }
    }
  }

  // ========== LLM 队列状态指示（第四阶段） ==========
  function updateLLMQueueStatus(status) {
    var queueInfoEl = document.getElementById('llm-queue-info');
    var statusBoxEl = document.getElementById('llm-status-box');
    var dotEl = document.getElementById('llm-dot');
    var statusTextEl = document.getElementById('llm-status');

    if (!queueInfoEl) return;

    if (status.generating) {
      // 忙：显示当前任务类型 + 队列长度
      var taskLabel = status.currentTaskType || 'LLM请求';
      var qLen = status.queueLength || 0;
      queueInfoEl.style.display = 'inline';
      queueInfoEl.textContent = '正在: ' + taskLabel + (qLen > 0 ? ' | 队列: ' + qLen : '');
      if (statusBoxEl) statusBoxEl.title = 'LLM状态：繁忙 | 当前任务：' + taskLabel + ' | 队列长度：' + qLen;
      // 视觉脉冲效果
      if (statusBoxEl && !statusBoxEl.classList.contains('llm-busy')) {
        statusBoxEl.classList.add('llm-busy');
      }
      // 同步更新仪表盘上的队列指示器
      var dashQueueSection = document.getElementById('dash-llm-queue-section');
      if (dashQueueSection) { dashQueueSection.style.display = ''; }
      var dashQueueText = document.getElementById('dash-llm-queue-text');
      if (dashQueueText) dashQueueText.textContent = '正在: ' + taskLabel + (qLen > 0 ? ' | 队列: ' + qLen : '');
    } else {
      // 空闲
      queueInfoEl.style.display = 'none';
      if (statusBoxEl) { statusBoxEl.classList.remove('llm-busy'); statusBoxEl.title = 'LLM状态：空闲'; }
      var dashQueueSection2 = document.getElementById('dash-llm-queue-section');
      if (dashQueueSection2) dashQueueSection2.style.display = 'none';
    }

    // 当 LLM 在线但空闲时，显示 ready 状态
    if (!status.generating && status.available && dotEl && dotEl.className.indexOf('active') >= 0) {
      if (statusTextEl) statusTextEl.textContent = 'LLM就绪';
    }
  }
  ...
  function _debouncedRenderCharts() {
    ...
    _chartDebounceTimer = setTimeout(function() {
      ...
      try { renderIncomeTrendChart(SGame.G); } catch(e) { SGame.DEBUG && console.error('[UI] chart income error:', e); }
      try { renderBizPieChart(SGame.G); } catch(e) { SGame.DEBUG && console.error('[UI] chart pie error:', e); }
    }, 200);
  }
  ...
  function switchPanel(name) {
    if (!['dashboard', 'region', 'business', 'npc', 'asset', 'achievement', 'stats', 'worldmap', 'tech', 'stock', 'ranking', 'milestone', 'intel', 'financial'].includes(name)) return;
    currentPanel = name;
    ...
    if (name === 'dashboard') {
      ...
      setTimeout(() => {
        try {
          ...
          SGame.DEBUG && console.error('[UI] switchPanel renderCenterPanel error:', e);
        }
  ...
  function renderCenterPanel(name) {
    ...
    switch (name) {
      ...
      case 'financial': renderFinancialPanel(panel); return;
    }
  }
  ...
  function renderWorldMap(panel) {
    ...
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent)">🗺 世界地图</div>';
    html += '</div>';
    ...
    domestic.forEach(cid => {
      ...
      const btnStyle = isCurrent
        ? 'border:2px solid var(--accent);background:rgba(245,158,11,0.15);'
        : isUnlocked
      ...
      html += `<div onclick="${isUnlocked ? "UI.switchCity('" + cid + "')" : ''}" style="${btnStyle}padding:12px 16px;border-radius:10px;text-align:center;min-width:90px;${isUnlocked ? 'cursor:pointer;' : ''}${isUnlocked ? '' : 'cursor:default;'}">
        ...
        ${isCurrent ? '<div style="font-size:10px;color:var(--accent);margin-top:2px;">● 当前</div>' : ''}
      </div>`;
    });
    html += '</div></div>';
    ...
    html += `<div style="margin-top:16px;padding:12px;background:var(--bg-hover);border-radius:8px;font-size:11px;color:var(--text-secondary);">
      已解锁城市: <b style="color:var(--accent)">${unlockedCount}</b>/10 |
      跨城协同加成: <b style="color:var(--green)">+${synergyPct}%</b>
    </div>`;
  ...
  function switchCity(cityId) {
    ...
    // 城市切换后强制刷新所有面板（绕过增量缓存）
    markDirty('dashboard');
    markDirty('regions');
    markDirty('businesses');
    markDirty('employees');
    markDirty('npcs');
    markDirty('charts');
    // 重置渲染缓存，确保 cityId 变化后的 bizKeys/regionKeys 不被旧缓存误判为无变化
    _renderCache = {
      dashboard: '', dashMoney: 0, dashIncome: 0, dashExpense: 0, dashEmpCount: 0, dashBizCount: 0,
      dashRep: 0, dashConn: 0, dashSent: 0, dashAssetCnt: 0, dashStockCnt: 0, dashStockPriceHash: '', dashResearchN: 0, dashBrandAw: 0, dashBrandRt: 0, dashFinancialCnt: 0,
      regions: '', regionKeys: '',
      employees: '', empKeys: '',
      businesses: '', bizKeys: '',
  ...
  function renderCitySelector() {
    ...
    return `<select id="city-selector" onchange="UI.switchCity(this.value)" style="background:var(--bg-primary);color:var(--accent);border:1px solid var(--border);border-radius:4px;font-size:11px;padding:2px 6px;">${opts}</select>`;
  }
  ...
  // ========== 昼夜主题切换 ==========
  var _lastTimeOfDay = null;
  function applyTimeOfDayTheme(G) {
    if (!G) { SGame.DEBUG && console.warn('[Theme] applyTimeOfDayTheme: G is null'); return; }
    if (G.gameHour === undefined || G.gameHour === null) { SGame.DEBUG && console.warn('[Theme] applyTimeOfDayTheme: gameHour is undefined'); return; }
    var hour = G.gameHour;
    var timeOfDay = (typeof SGame.getTimeOfDay === 'function') ? SGame.getTimeOfDay(hour) : 'day';
    var body = document.body;
    if (!body) return;
    // 手动模式下不干预：清除自动时段类，完全由用户 toggleTheme 控制
    if (localStorage.getItem('shanghaifc-theme-mode') === 'manual') {
      body.classList.remove('time-dawn', 'time-day', 'time-dusk', 'time-night');
      _lastTimeOfDay = null;
      return;
    }
    // 仅当时段变化时才操作 DOM（避免无谓的 class 抖动）
    if (timeOfDay === _lastTimeOfDay) return;
    SGame.DEBUG && console.log('[Theme] 时段切换: ' + (_lastTimeOfDay || '初始') + ' -> ' + timeOfDay + ' (gameHour=' + hour + ')');
    // 移除旧时段类和 light-mode（自动模式下无需保留手动亮色偏好）
    body.classList.remove('time-dawn', 'time-day', 'time-dusk', 'time-night');
  ...
  function _shouldSkipDashboard(G) {
    ...
    // 计算股价哈希，检测任何股价变动
    var stockPriceHash = G.stockPrices ? Object.entries(G.stockPrices).map(function(e) { return e[0] + ':' + e[1]; }).join('|') : '';
    var researchTotal = 0;
    ...
    var brandAw = G.brand ? Math.round(G.brand.awareness || 0) : 0;
    var brandRt = G.brand ? Math.round(G.brand.rating || 0) : 0;
    var financialCnt = G.financialReports ? G.financialReports.length : 0;
    ...
    if (moneyDelta < 200 &&
        ...
        stockPriceHash === _renderCache.dashStockPriceHash &&
        researchTotal === _renderCache.dashResearchN &&
        brandAw === _renderCache.dashBrandAw &&
        brandRt === _renderCache.dashBrandRt &&
        financialCnt === _renderCache.dashFinancialCnt) {
      return true;
    }
    ...
    _renderCache.dashStockPriceHash = stockPriceHash;
    _renderCache.dashResearchN = researchTotal;
    _renderCache.dashBrandAw = brandAw;
    _renderCache.dashBrandRt = brandRt;
    _renderCache.dashFinancialCnt = financialCnt;
    return false;
  }
  ...
  function _doRenderAll() {
    ...
    var dirtyRender = function(name, fn, skipFn, flag) {
      ...
      try {
        ...
        SGame.DEBUG && console.error('[商海浮沉] renderAll/' + name + ' error:', e);
        if (typeof showToast === 'function') showToast('面板渲染失败，请刷新页面', 'error');
      }
      dirty[flag] = false;
    };
    ...
    // #6 员工列表：仅数量/角色/忠诚度/疲劳度/技能变化时重绘
    var empKeys = G.employees ? G.employees.map(function(e) { return e.id + ':' + (e.role || '') + ':' + (e.loyalty|0) + ':' + (e.fatigue|0) + ':' + (e.skill|0); }).join(',') : '';
    dirtyRender('employeeList', renderEmployeeList, function() { return _shouldSkipSimple('employees', empKeys); }, 'employees');

    // #6 业务列表：仅等级/区域变化时重绘
    var bizKeys = (G.businesses ? Object.entries(G.businesses).filter(function(e) { return e[1].level > 0; }).map(function(e) { return e[0] + ':' + e[1].level + ':' + (e[1].region || ''); }).join(',') : '') + '|city:' + (G.currentCityId || '');
    ...
    try { if (!_sfLog()) renderEventLog(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/eventLog error:', e); if (typeof showToast === 'function') showToast('事件日志渲染失败，请刷新页面', 'error'); }

    // #6 统计面板：几乎每tick变化但渲染轻量，保持
    try { renderStats(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/stats error:', e); if (typeof showToast === 'function') showToast('统计面板渲染失败，请刷新页面', 'error'); }
    ...
    try { renderActDisplay(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/actDisplay error:', e); }
    try { renderHotSearch(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/hotSearch error:', e); }
    try { renderHireButton(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/hireButton error:', e); }
    // 重新显示搁置/延迟事件卡片
    if (typeof EventSystem !== 'undefined' && EventSystem.showDeferredEvents) {
      try { EventSystem.showDeferredEvents(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/showDeferredEvents error:', e); }
    }
    try { renderSynergyStatus(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/synergyStatus error:', e); }
    try { renderManualButton(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/manualButton error:', e); }
    try { renderClock(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/clock error:', e); }
    try { renderAutoButton(); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/autoButton error:', e); }

    // 实时面板刷新：如果用户正在查看需要实时更新的子面板，触发重绘
    if (currentPanel && currentPanel !== 'dashboard') {
      if (currentPanel === 'stock') {
        // 股票面板：交易操作后需要全量渲染，否则增量更新股价
        if (_stockNeedsFullRefresh || !document.querySelector('#center-panel #stock-A001')) {
          _stockNeedsFullRefresh = false;
          try { renderCenterPanel(currentPanel); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/centerPanel(stock) error:', e); }
        } else if (typeof _updateStockPrices === 'function') {
          try { _updateStockPrices(); } catch(e) { SGame.DEBUG && console.error('updateStockPrices error:', e); }
        }
      } else {
        try { renderCenterPanel(currentPanel); } catch(e) { SGame.DEBUG && console.error('[商海浮沉] renderAll/centerPanel(' + currentPanel + ') error:', e); }
      }
    }
  }
    ...
    overlay.innerHTML = `
      <div class="modal" style="text-align:center;">
        ...
        <div style="font-size:13px;color:var(--text-secondary);margin:12px 0;line-height:1.8;">
          你离开了 <b style="color:var(--accent)">${(offlineData.hours ?? 0).toFixed(1)} 小时</b><br>
          期间经过 <b style="color:var(--accent)">${offlineData.ticks ?? 0} Tick</b><br>
          产生收益: <b style="color:var(--green);font-size:18px;">+${SGame.formatMoney(offlineData.income ?? 0)}</b>
        </div>
        <div style="display:flex;gap:10px;justify-content:center;margin-top:16px;">
          <button class="btn" style="font-size:13px;padding:8px 24px;background:var(--accent);color:#0a0e1a;" onclick="UI.claimOfflineIncome(this)">领取收益</button>
  ...
  function renderStats() {
    ...
    el.innerHTML = `
      <div style="font-size:13px;font-weight:600;color:var(--accent);margin-bottom:8px">${escapeHtml(G.name) || '未命名'}</div>
      <div class="stat-row"><span class="stat-label">头衔</span><span class="stat-value" style="color:var(--accent)">${rankIcon} ${G.rank || '个体户'}</span></div>
      <div class="stat-row"><span class="stat-label">资产</span><span class="stat-value" style="color:var(--accent)">${SGame.formatMoney(G.money)}</span></div>
      ...
      <div class="stat-row"><span class="stat-label">人脉</span><span class="stat-value" style="color:var(--accent)">${G.connections ?? 0}</span></div>
  ...
  function renderRegions(optEl) {
    ...
    Object.values(REGIONS).forEach(r => {
      ...
      if (!r.unlocked && !isUnlocked) {
        html += `<div class="stat-row" style="opacity:0.4;font-size:11px;min-height:26px;align-items:center"><span class="stat-label">🔒 ${r.name}</span><span class="stat-value" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:70px">未解锁</span></div>`;
        return;
      }
      html += `<div class="stat-row" style="font-size:11px;min-height:26px;align-items:center">
        <span class="stat-label">${r.name}</span>
        <span class="stat-value" style="font-size:10px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:70px">${getRegionBonusDesc(r.bonus)}</span>
      </div>`;
    });
    el.innerHTML = html;
  }
  ...
  function renderBusinessList(optEl) {
    ...
    BUSINESS_DEFS.forEach(b => {
      ...
      if (lv > 0) {
        ...
        if (ms !== undefined) {
          ...
          marketShareHtml = `<div style="font-size:10px;color:var(--accent);margin-top:2px">市场份额: ${msPct}%</div>`;
        }
        const sc = G.supplyChain ? G.supplyChain[b.id] : undefined;
        if (sc) {
          const upColor = sc.upstream === 'normal' ? 'var(--green)' : 'var(--red)';
          const downColor = sc.downstream === 'normal' ? 'var(--green)' : 'var(--red)';
      ...
      html += `<div data-biz-id="${b.id}" style="padding:10px 0;border-bottom:1px solid var(--border)" class="${lvColorClass}">
        ...
        ${lv > 0 ? `<div style="font-size:10px;color:var(--accent);margin-top:2px">收益: ${(SGame.calcBizIncome ? SGame.calcBizIncome(b.id, G) : def.income).toFixed(1)}万/年</div>` : ''}
        ${marketShareHtml}
        ${supplyChainHtml}
        ${regionSelect}
        <div style="display:flex;gap:6px;margin-top:6px">
          ...
          ${nextDef ? (() => {
            const baseCost = nextDef.cost * WAN;
            const cost = (G.origin === 'rich2nd') ? Math.floor(baseCost * 0.8) : baseCost;
            ...
            if (nextDef.reqCond) {
              if (nextDef.reqCond.techLv) {
                const treeId = b && b.techTree ? b.techTree : 'digital';
            ...
            if (canAfford && condMet) {
              return `<button class="btn" style="font-size:10px;padding:3px 8px" onclick="UI.upgradeBusiness('${b.id}')">升级 (${SGame.formatMoney(cost)})</button>`;
            } else {
              const hint = !canAfford ? `升级 (${SGame.formatMoney(cost)})` : `升级 (${condHint})`;
          ...
          ${lv > 0 ? `<button class="btn" style="font-size:10px;padding:3px 8px;background:var(--accent);color:#0a0e1a;" onclick="UI.upgradeBusinessMax('${b.id}')">一键升级</button>` : ''}
    ...
    // 增量渲染业务列表
    _patchList(el, html, 'biz-id');
    // 已并购业务列表
    var maList = (SGame && typeof SGame.getMAList === 'function') ? SGame.getMAList() : [];
    // 清理旧的并购区域
    var oldMA = el.querySelector('.ma-section');
    if (oldMA) oldMA.remove();
    if (maList && maList.length > 0) {
      var maHtml = '<div class="ma-section" style="margin-top:16px;padding-top:12px;border-top:2px solid var(--accent);"><div style="font-size:13px;font-weight:600;color:var(--accent);margin-bottom:8px;">🤝 已并购业务</div>';
      maList.forEach(function(b) {
        maHtml += '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px;">' +
          ...
          '<span style="color:var(--green);font-weight:600;">+' + SGame.formatMoney(b.revenuePerTick || 0) + '/Tick</span>' +
          '</div>';
      });
      ...
      el.insertAdjacentHTML('beforeend', maHtml);
    }
  }
  ...
  function openBusiness(bizId) {
    try {
      ...
      let openCost = (lv1.cost || 0) * WAN;
      if (G.origin === 'rich2nd') openCost = Math.floor(openCost * 0.8);
      if (openCost > 0 && G.money < openCost) {
        try { EventSystem.addLog(`${bDef.icon} ${bDef.name} 资金不足，无法开业（需要${SGame.formatMoney(openCost)}）。`); } catch(e) { SGame.DEBUG && console.error('[ui] openBusiness addLog failed:', e && e.message); }
        return;
      }
      if (openCost > 0) G.money -= openCost;
      state.level = 1;
      ...
      try { EventSystem.addLog(`${bDef.icon} ${bDef.name} 开业了！`); } catch(e) { SGame.DEBUG && console.error('[ui] openBusiness addLog failed:', e && e.message); }
      markDirty('businesses');
      markDirty('dashboard');
      renderAll();
    } catch(e) {
      SGame.DEBUG && console.error('[商海浮沉] openBusiness error:', e);
    }
  }
  ...
  function upgradeBusiness(bizId) {
    ...
    let cost = next.cost * WAN;
    if (G.origin === 'rich2nd') cost = Math.floor(cost * 0.8);
    if (G.money < cost) { EventSystem.addLog(`${bDef.icon} ${bDef.name} 资金不足，升级需要 ${SGame.formatMoney(cost)}。`); return; }
    G.money -= cost;
    state.level++;
    // 同步到当前城市的 businesses
    if (G.cities && G.currentCityId) {
  ...
  function renderSynergyStatus() {
    ...
    el.innerHTML = '<span style="font-size:10px;color:var(--text-muted);margin-right:6px;white-space:nowrap;line-height:1.5;">联动:</span>' +
      items.map(it => {
        const color = it.positive ? 'var(--green)' : 'var(--red)';
        const sign = it.positive ? '+' : '';
        return `<span style="font-size:10px;color:${color};background:${it.positive ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)'};padding:1px 5px;border-radius:4px;white-space:nowrap;line-height:1.5;">${it.label}:${sign}${it.value}</span>`;
      }).join('');
  }
  ...
  function renderDashboard() {
    ...
    const rawExpense = (G.employees || []).reduce((s, e) => s + calcActualSalary(e.baseSalary ?? e.salary, G) * WAN, 0);
    ...
    // 增量更新：若DOM已存在则仅更新数值textContent，避免全量innerHTML重建
    // 增量更新LLM队列状态指示器
    var dashQueueEl = el.querySelector('#dash-llm-queue-section');
    if (dashQueueEl) {
      var qInfoEl = document.getElementById('llm-queue-info');
      if (qInfoEl && qInfoEl.style.display !== 'none') {
        dashQueueEl.style.display = '';
        var dashQueueText = el.querySelector('#dash-llm-queue-text');
        if (dashQueueText) dashQueueText.textContent = qInfoEl.textContent || '';
      } else {
        dashQueueEl.style.display = 'none';
      }
    }

    var dashMoneyEl = el.querySelector('#dash-money');
    if (dashMoneyEl) {
      dashMoneyEl.textContent = SGame.formatMoney(G.money ?? 0);
      var dashAssetsEl = el.querySelector('#dash-total-assets');
      if (dashAssetsEl) dashAssetsEl.textContent = SGame.formatMoney(G.money ?? 0);
      var dashIncomeEl = el.querySelector('#dash-income');
      if (dashIncomeEl) dashIncomeEl.textContent = SGame.formatMoney(income);
      var dashExpenseEl = el.querySelector('#dash-expense');
      if (dashExpenseEl) dashExpenseEl.textContent = SGame.formatMoney(expense);
      var dashMaintEl = el.querySelector('#dash-maintenance');
      if (dashMaintEl) dashMaintEl.textContent = SGame.formatMoney(maintenanceCost);
      var dashEmpCountEl = el.querySelector('#dash-emp-count');
      if (dashEmpCountEl) {
        dashEmpCountEl.textContent = (G.employees || []).length;
        dashEmpCountEl.style.color = (G.employees || []).length > SGame.getEmpMax() ? 'var(--red)' : 'var(--accent)';
      }
      var dashEmpMaxEl = el.querySelector('#dash-emp-max');
      if (dashEmpMaxEl) dashEmpMaxEl.textContent = SGame.getEmpMax();
      var dashBizCountEl = el.querySelector('#dash-biz-count');
      if (dashBizCountEl) dashBizCountEl.textContent = Object.values(G.businesses || {}).filter(function(b) { return b.level > 0; }).length;
      // 增量更新托管运行统计（auto-stats 每次 tick 都可能变化）
      var badgesEl = el.querySelector('#auto-badges');
      if (badgesEl && G.autoMode && G.autoMode.enabled) {
        var am = G.autoMode;
        var dot = '<span style="color:var(--green);">●</span>';
        var gray = '<span style="color:var(--text-muted);">○</span>';
        var b = [];
        if (am.eventDecide) b.push(dot + '事件'); else b.push(gray + '事件');
        if (am.autoOpenBusiness || am.autoUpgradeBusiness) b.push(dot + '业务');
        if (am.autoHire) b.push(dot + (typeof SGame.isHRManaged === 'function' && SGame.isHRManaged() ? '🏢员工' : '员工')); else b.push(gray + '员工');
        if (am.autoUnlockRegion) b.push(dot + '区域');
        if (am.autoResearch) b.push(dot + '研发');
        if (am.autoInvest) b.push(dot + '投资');
        if (am.autoRepay || am.autoLoan) b.push(dot + '贷款');
        if (am.autoGift) b.push(dot + '社交');
        badgesEl.innerHTML = b.join(' · ');
      }
      var astatEl = el.querySelector('#auto-stats');
      if (astatEl) astatEl.innerHTML = renderDashboardAutoStats(G);
      // 增量更新底部额外行（贷款/股票市值）
      renderDashboardExtraRow(G);
      // 增量更新子卡片（股票/研发/品牌/舆情/资产/财报/收支明细/图表/趋势）
      var scEl = el.querySelector('#dash-stock-card');
      if (scEl) scEl.innerHTML = renderDashboardStockCard(G);
      var fcEl = el.querySelector('#dash-financial-card');
      if (fcEl) fcEl.innerHTML = renderDashboardFinancialCard(G);
      var rcEl = el.querySelector('#dash-research-card');
      if (rcEl) rcEl.innerHTML = renderDashboardResearchCard(G);
      var leftSec = document.getElementById('dash-left-section');
      if (leftSec) leftSec.innerHTML = renderDashboardBreakdownCard(G, renderIncomeBreakdown());
      var leftMarket = document.getElementById('dash-left-market');
      if (leftMarket) leftMarket.innerHTML = renderDashboardLeftMarket(G);
      var chEl = el.querySelector('#dash-chart');
      if (chEl) chEl.innerHTML = renderMiniAssetChart(G);
      var trEl = el.querySelector('#dash-trend');
      if (trEl) trEl.innerHTML = renderAssetTrend(G);
      var tiEl = el.querySelector('#dash-time-info');
      if (tiEl) tiEl.textContent = getTimeDisplay(G);
      return;
    }
    ...
    el.innerHTML = `
      <div class="dash-section">
        <div class="dash-section-header">资金概览</div>
        <div class="dash-card" style="grid-column:span 2;">
          <div class="dash-label">${rankIcon} ${G.rank || '个体户'}  |  ${citySelectHtml}</div>
          <div class="dash-value" style="color:var(--accent);font-size:28px;"><span id="dash-money">${SGame.formatMoney(G.money ?? 0)}</span> <span id="dash-trend">${trendHtml}</span></div>
          <div class="dash-sub"><span id="dash-time-info">${timeInfo}</span> |  当前总资产 <span id="dash-total-assets">${SGame.formatMoney(G.money ?? 0)}</span></div>
          ${autoStatusHtml}
          <div id="dash-chart">${chartHtml}</div>
        </div>
        <div class="dash-card">
          <div class="dash-label">Tick收益</div>
          <div class="dash-value" style="color:var(--green)">+<span id="dash-income">${SGame.formatMoney(income)}</span></div>
          <div class="dash-sub">每${(tickMs/1000).toFixed(0)}秒</div>
        </div>
        <div class="dash-card">
          <div class="dash-label">工资支出</div>
          <div class="dash-value" style="color:var(--red)">-<span id="dash-expense">${SGame.formatMoney(expense)}</span></div>
          <div class="dash-sub">每Tick</div>
        </div>
        <div class="dash-card">
          <div class="dash-label">维护成本</div>
          <div class="dash-value" style="color:var(--red);font-size:18px;">-<span id="dash-maintenance">${SGame.formatMoney(maintenanceCost)}</span></div>
          <div class="dash-sub">每Tick</div>
        </div>
        <div class="dash-card">
          <div class="dash-label">员工数</div>
          <div class="dash-value" id="dash-emp-count" style="color:${(G.employees || []).length > SGame.getEmpMax() ? 'var(--red)' : 'var(--accent)'}" title="员工上限由管理能力、运营业务数、已解锁区域和资产规模共同决定">${(G.employees || []).length}</div>
          <div class="dash-sub">上限 <span id="dash-emp-max">${SGame.getEmpMax()}</span></div>
        </div>
        <div class="dash-card">
          <div class="dash-label">业务数</div>
          <div class="dash-value" id="dash-biz-count" style="color:var(--accent)">${Object.values(G.businesses || {}).filter(b => b.level > 0).length}</div>
          <div class="dash-sub">已开业</div>
        </div>
      </div>
      <div class="dash-section" id="dash-llm-queue-section" style="display:none;">
        <div class="dash-section-header">LLM 任务队列</div>
        <div class="dash-card llm-queue-card">
          <div style="display:flex;align-items:center;gap:8px;">
            <span class="llm-dot active" style="width:8px;height:8px;flex-shrink:0;"></span>
            <span id="dash-llm-queue-text" style="font-size:12px;color:var(--accent-gold);"></span>
          </div>
        </div>
      </div>
      <div class="dash-section">
        <div class="dash-section-header">运营指标</div>
        <div id="dash-stock-card">${stockCardHtml}</div>
        <div id="dash-financial-card">${renderDashboardFinancialCard(G)}</div>
        <div id="dash-research-card">${researchCardHtml}</div>
      </div>
      <div id="dashboard-extra"></div>
    `;
    ...
    document.querySelectorAll('.dash-section').forEach(function(s) {
      if (!s.querySelector('.dash-card')) s.style.display = 'none';
    });
  }
  ...
  function renderDashboardAutoStatus(G) {
    ...
    if (G.autoMode && G.autoMode.enabled) {
      ...
      autoStatusHtml = '<div class="auto-banner">' +
        ...
        '<div id="auto-badges" style="display:flex;align-items:center;gap:3px;flex-wrap:wrap;">' + badges.join(' · ') + '</div></div></div>';
      ...
      if (stats && stats.totalTicks > 0) {
        ...
        autoStatusHtml += '<div id="auto-stats" style="font-size:10px;color:var(--text-muted);padding:4px 2px;display:flex;gap:8px;flex-wrap:wrap;">' +
  ...
  // ========== 托管运行统计的增量渲染（供 renderDashboard 增量路径复用） ==========
  function renderDashboardAutoStats(G) {
    var html = '';
  ...
  function renderDashboardStockCard(G) {
    ...
    const stockPnlColor = stockPnl >= 0 ? 'var(--green)' : 'var(--red)';
    const stockPnlSign = stockPnl >= 0 ? '+' : '';
    if (stockCount > 0) {
      return '<div class="dash-card compact">' +
        '<div class="dash-label">股票持仓</div>' +
        '<div class="dash-value" style="color:var(--accent);font-size:18px;">' + SGame.formatMoney(stockPortVal) + '</div>' +
  ...
  function renderDashboardFinancialCard(G) {
    var reports = G.financialReports || [];
    if (reports.length === 0) return '';
    var latest = reports[reports.length - 1];
    var netColor = latest.netProfit >= 0 ? 'var(--green)' : 'var(--red)';
    var netSign = latest.netProfit >= 0 ? '+' : '';
    var empDelta = latest.endEmployees - latest.startEmployees;
    var empDeltaStr = empDelta > 0 ? ' +' + empDelta : (empDelta < 0 ? ' ' + empDelta : '');
    var bizDelta = latest.endBusinesses - latest.startBusinesses;
    var bizDeltaStr = bizDelta > 0 ? ' +' + bizDelta : '';
    return (
      '<div class="dash-card compact" style="cursor:pointer;" onclick="UI.switchPanel(\'financial\')" title="点击查看详细财报">' +
      '<div class="dash-label">年度财报 #' + latest.id + '</div>' +
      '<div class="dash-value" style="color:' + netColor + ';font-size:18px;">' + netSign + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(latest.netProfit) : latest.netProfit.toLocaleString()) + '</div>' +
      '<div class="dash-sub">净利 | 员工' + empDeltaStr + ' | 业务' + bizDeltaStr + '</div>' +
      '</div>'
    );
  }
  ...
  function renderDashboardResearchCard(G) {
    ...
    if (G.activeResearch && G.activeResearch.techId) {
      ...
      if (arRoute) activeResearchName = (arRoute.icon ? arRoute.icon + ' ' : '') + arRoute.name + ' Lv.' + (ar.level);
    }
    if (researchTotal > 0 && (researchCompleted > 0 || activeResearchName)) {
      ...
      return '<div class="dash-card compact">' +
        ...
        '<div class="dash-value" style="color:var(--accent)">' + researchCompleted + '/' + researchTotal + '</div>' +
        '<div class="stat-bar" style="margin-top:4px;"><div class="stat-bar-fill" style="width:' + researchPct + '%;background:var(--accent)"></div></div>' +
  ...
  function renderDashboardBrandCard(G) {
    const brand = G.brand;
    if (!brand) return '';
    const awarenessColor = brand.awareness >= 80 ? 'var(--accent)' : brand.awareness >= 50 ? 'var(--accent)' : 'var(--red)';
    const ratingColor = brand.rating >= 80 ? 'var(--green)' : brand.rating >= 50 ? 'var(--accent)' : 'var(--red)';
    return '<div class="dash-card compact">' +
      '<div class="dash-label">品牌口碑</div>' +
      '<div class="brand-bar-row"><span class="stat-bar-label" style="color:var(--text-secondary)">知名度</span>' +
      '<div class="stat-bar"><div class="stat-bar-fill" style="width:' + brand.awareness + '%;background:' + awarenessColor + '"></div></div>' +
      '<span class="stat-bar-label" style="color:' + awarenessColor + '">' + Math.round(brand.awareness) + '/100</span></div>' +
      '<div class="brand-bar-row"><span class="stat-bar-label" style="color:var(--text-secondary)">口碑</span>' +
      '<div class="stat-bar"><div class="stat-bar-fill" style="width:' + brand.rating + '%;background:' + ratingColor + '"></div></div>' +
      '<span class="stat-bar-label" style="color:' + ratingColor + '">' + Math.round(brand.rating) + '/100</span></div>' +
      (brand.crisisHistory && brand.crisisHistory.length > 0
        ? '<div class="dash-sub" style="color:var(--red)">危机 ' + brand.crisisHistory.length + ' 次</div>'
        : '<div class="dash-sub">品牌健康</div>') +
      '</div>';
  }
  ...
  function renderDashboardSentimentCard(G) {
    ...
    if (sentiment >= 80) { sentColor = 'var(--red)'; sentIcon = '●'; sentLabel = '极度乐观'; }
    else if (sentiment >= 65) { sentColor = 'var(--accent)'; sentIcon = '●'; sentLabel = '偏乐观'; }
    else if (sentiment >= 45) { sentColor = '#89b0d0'; sentIcon = '●'; sentLabel = '中性'; }
    else if (sentiment >= 25) { sentColor = 'var(--accent)'; sentIcon = '●'; sentLabel = '偏悲观'; }
    else { sentColor = 'var(--green)'; sentIcon = '●'; sentLabel = '极度悲观'; }
    return '<div class="dash-card compact">' +
      '<div class="dash-label">市场情绪</div>' +
      '<div class="dash-value" style="color:' + sentColor + '"><span class="sentiment-indicator">' + sentIcon + '</span>' + sentiment + '</div>' +
  ...
  function renderDashboardAssetCard(G) {
    ...
    return '<div class="dash-card compact">' +
      ...
      '<div class="dash-value" style="color:var(--accent);font-size:22px;font-weight:700">' + SGame.formatMoney(assetVal) + '</div>' +
  ...
  function renderDashboardBreakdownCard(G, breakdownHtml) {
    ...
    return ''
      + '<div class="dash-card compact">'
        ...
        + '<div class="dash-value" style="color:var(--accent)">' + repCount + '</div>'
        + '<div class="dash-sub">/100</div>'
      + '</div>'
      + '<div class="dash-card compact">'
        + '<div class="dash-label">人脉</div>'
        + '<div class="dash-value" style="color:var(--accent)">' + connCount + '</div>'
  ...
  function renderDashboardLeftMarket(G) {
    return renderDashboardAssetCard(G)
      + renderDashboardBrandCard(G)
      + renderDashboardSentimentCard(G);
  }

  function renderDashboardExtraRow(G) {
    const extraEl = document.getElementById('dashboard-extra');
    if (!extraEl) return;
    ...
    if (portfolioVal > 0) {
      ...
      const pnlSign = pnl >= 0 ? '+' : '';
      extraHtml += '<div style="font-size:11px;color:var(--text-secondary);margin-bottom:2px;">股票市值: <b style="color:var(--accent)">' + SGame.formatMoney(portfolioVal) + '</b> | 盈亏: <b style="color:' + pnlColor + '">' + pnlSign + SGame.formatMoney(pnl) + '</b></div>';
    }
    if (G.loans && G.loans.length > 0) {
      const totalLoan = G.loans.reduce((s, l) => s + l.amount, 0);
      extraHtml += '<div style="font-size:11px;color:var(--text-secondary);margin-bottom:2px;">贷款余额: <b style="color:var(--red)">' + SGame.formatMoney(totalLoan) + '</b> (' + G.loans.length + '笔)</div>';
    }
  ...
  function renderIncomeBreakdown() {
    const G = SGame.G;
    const breakdown = [];
    if (typeof BUSINESS_DEFS !== 'undefined') BUSINESS_DEFS.forEach(bDef => {
  ...
  function renderEmpSpecs(emp, roleDef) {
    ...
    roleDef.specialization.forEach(spec => {
      ...
      for (let l = 1; l <= spec.maxLv; l++) {
        lvHtml += `<span style="color:${l <= curLv ? 'var(--accent)' : 'var(--border)'}">◆</span>`;
      }
      const btnStyle = curLv < spec.maxLv
        ? 'cursor:pointer;background:var(--accent);color:#fff;border:none;padding:1px 5px;border-radius:3px;font-size:9px'
  ...
  function renderEmployeeList() {
    ...
    employees.forEach(emp => {
      ...
      const loyaltyColor = emp.loyalty >= 50 ? 'var(--green)' : emp.loyalty >= 20 ? 'var(--accent)' : 'var(--red)';
      const fatigue = emp.fatigue || 0;
      const skill = emp.skill || 1;
      const fatigueColor = fatigue >= 70 ? 'var(--red)' : fatigue >= 40 ? 'var(--accent)' : 'var(--green)';
      // 实习生状态展示
      var internBadge = '';
      var internProgress = '';
      if (emp.isIntern && !emp.internConverted) {
        ...
        internBadge = ' <span style="font-size:10px;background:var(--accent);color:#fff;padding:1px 5px;border-radius:3px;vertical-align:middle">实习中</span>';
        internProgress = '<div style="margin-top:2px;font-size:10px;color:var(--accent)">' +
          '📋 实习进度: <div style="display:inline-block;width:60px;height:5px;background:var(--border);border-radius:3px;vertical-align:middle">' +
          '<div style="width:' + progress + '%;height:100%;background:var(--accent);border-radius:3px"></div></div>' +
          ' ' + progress.toFixed(0) + '%（剩余' + ticksLeft + 'tick）→ ' + targetLabel + '</div>';
      } else if (emp.internConverted) {
        internBadge = ' <span style="font-size:10px;background:var(--green);color:#fff;padding:1px 5px;border-radius:3px;vertical-align:middle">✅ 已转正</span>';
      }
      // 工资显示（实习生打折）
      ...
      var salaryNote = (emp.isIntern && !emp.internConverted) ? '万/年<span style="font-size:9px;color:var(--accent)">(实习5折)</span>' : '万/年';
      // 属性展示
      var attrLine = '';
      if (emp.attrs && typeof SGame.EMP_ATTRIBUTES !== 'undefined') {
        ...
        Object.entries(emp.attrs).forEach(function(entry) {
          ...
          if (def) {
            var valColor = val >= 70 ? 'var(--green)' : val >= 40 ? 'var(--accent)' : 'var(--text-muted)';
      ...
      html += `<div data-emp-id="${emp.id}" style="padding:8px 0;border-bottom:1px solid var(--border)">
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <div style="display:flex;gap:4px;">
            <button class="btn" style="font-size:10px;padding:3px 8px;background:var(--accent);color:#0a0e1a;" onclick="UI.trainEmployee(${emp.id})" title="培训提升技能">📚 培训</button>
            <button class="btn" style="font-size:10px;padding:3px 8px;background:var(--green);color:#fff;" onclick="UI.restEmployee(${emp.id})" title="休息恢复疲劳">😴 休息</button>
            <button class="btn" style="font-size:10px;padding:3px 8px;opacity:0.6" onclick="UI.fireEmployee(${emp.id})">解雇</button>
          </div>
        </div>
      </div>`;
    });
    _patchList(el, html, 'emp-id');
  }

  // ========== HR 统管：部门面板渲染 ==========
  function renderHRManagedPanel(el, G) {
    const depts = SGame.calcDeptStats();
    let html = '<div style="font-size:10px;color:var(--accent);margin-bottom:8px;font-weight:600">🏢 HR 部门统管模式</div>';
    Object.entries(depts).forEach(([roleId, d]) => {
      const loyalCol = d.avgLoyalty >= 50 ? 'var(--green)' : d.avgLoyalty >= 20 ? 'var(--accent)' : 'var(--red)';
      const fatigCol = d.avgFatigue >= 70 ? 'var(--red)' : d.avgFatigue >= 40 ? 'var(--accent)' : 'var(--green)';
      html += `<div data-emp-id="dept-${roleId}" style="padding:6px 0;border-bottom:1px solid var(--border);cursor:pointer;" onclick="UI.toggleDeptDetail('${roleId}')">
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <div style="display:flex;gap:6px;align-items:center">
            <button class="btn" style="font-size:9px;padding:2px 8px;background:var(--accent);color:#0a0e1a;" onclick="event.stopPropagation();UI.batchTrainDept('${roleId}')">📚 团训</button>
            <button class="btn" style="font-size:9px;padding:2px 8px;background:var(--accent);color:#0a0e1a;" onclick="event.stopPropagation();UI.batchHireDept('${roleId}')">+ 扩招</button>
          </div>
        </div>
        ...
        <div id="dept-detail-${roleId}" style="display:none;margin-top:4px;padding-left:12px;border-left:2px solid var(--accent)"></div>
      </div>`;
    });
  ...
  function toggleDeptDetail(roleId) {
    ...
    if (detailEl.style.display === 'none' || !detailEl.style.display) {
      ...
      deptEmps.forEach(emp => {
        const loyalCol = emp.loyalty >= 50 ? 'var(--green)' : emp.loyalty >= 20 ? 'var(--accent)' : 'var(--red)';
        const fatCol = (emp.fatigue || 0) >= 70 ? 'var(--red)' : (emp.fatigue || 0) >= 40 ? 'var(--accent)' : 'var(--green)';
        // 属性展示
        var attrLine = '';
        if (emp.attrs && typeof SGame.EMP_ATTRIBUTES !== 'undefined') {
  ...
  function renderHireButton() {
    ...
    if (count) {
      ...
      count.style.color = overMax ? 'var(--red)' : '';
    }
    if (max) {
      max.textContent = empMax;
      max.style.color = overMax ? 'var(--red)' : '';
  ...
  async function openHireModal() {
    ...
    hireCandidates.forEach((c, i) => {
      ...
      if (LLM.available && c.attrs) {
        ...
        const evalPromise = LLM.generate(prompt, 0.7, undefined, {
          taskType: '候选人评价',
          priority: LLM.PRIORITY ? LLM.PRIORITY.LOW : 2
        });
  ...
  function renderHRHirePanel(container) {
    const G = SGame.G;
    const depts = SGame.calcDeptStats();
    let html = '<div style="font-size:13px;font-weight:600;margin-bottom:12px;color:var(--accent)">🏢 HR 部门扩招</div>';
    ...
    EMP_ROLES.forEach(r => {
      ...
      const estCost = displaySalary * (typeof WAN !== 'undefined' ? WAN : 10000) * 2 * CONFIG.HR_HIRE_DISCOUNT; // 扩招2人
      const internNote = (r.id === 'intern' && r.internConvertTo) ? '<div style="font-size:9px;color:var(--accent);margin-top:2px">📋 实习期后可转正为：' + r.internConvertTo.map(function(rid) { var rd = EMP_ROLES.find(function(x) { return x.id === rid; }); return rd ? rd.name : rid; }).join('、') + '</div>' : '';
      html += `<div style="padding:10px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px;cursor:pointer;"
        ...
        <div style="font-size:10px;color:var(--text-muted);margin-top:4px">
          预估月薪: ${displaySalary.toFixed(1)}万/人${r.id === 'intern' ? '<span style="color:var(--accent)">(实习5折)</span>' : ''} | 招聘成本: ${SGame.formatMoney(estCost)}（HR折扣）
        </div>
        ${internNote}
      </div>`;
    });
  ...
  function renderHireCards(container) {
    const G = SGame.G;
    // 批量招聘UI (功能4)
    let batchHtml = '<div style="padding:10px;border:1px solid var(--accent);border-radius:8px;margin-bottom:12px;background:rgba(245,158,11,0.05);">';
    batchHtml += '<div style="font-size:12px;font-weight:600;color:var(--accent);margin-bottom:8px;">👥 批量招聘</div>';
    ...
    batchHtml += '</select>';
    batchHtml += '<button class="btn" style="font-size:11px;padding:4px 12px;background:var(--accent);color:#0a0e1a;" onclick="UI.batchHire()">批量录用</button>';
    batchHtml += '</div></div>';

    // 属性条渲染辅助函数
    function attrBar(value, maxVal) {
      const pct = Math.min(100, Math.max(0, (value / maxVal) * 100));
      const color = getAttrColor(pct);
    ...
    let cardsHtml = hireCandidates.map((c, i) => {
        ...
        Object.entries(c.attrs).forEach(function(entry) {
          ...
          if (def) {
            attrsHtml += '<div style="display:flex;align-items:center;gap:2px;">' +
              ...
              '<span style="font-weight:600;color:' + getAttrColor(val) + '">' + val + '</span>' +
              attrBar(val, 100) +
              '</div>';
          }
        });
      ...
      if (c.role === 'intern') {
        ...
        internNote = '<div style="font-size:10px;color:var(--accent);margin-top:4px;padding:4px 6px;background:rgba(6,182,212,0.08);border-radius:4px">' +
      ...
      <div style="padding:12px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">
        ...
        ${attrsHtml}
        <div id="hire-eval-${i}" style="font-size:11px;color:var(--accent);margin:2px 0;min-height:16px;"></div>
        <div style="display:flex;gap:12px;font-size:12px">
          <span>工资: ${c.role === 'intern' ? (calcActualSalary(c.baseSalary, G) * (CONFIG.INTERN_SALARY_RATIO || 0.5)).toFixed(1) : calcActualSalary(c.baseSalary, G)}万/月${c.role === 'intern' ? '<span style="font-size:9px;color:var(--accent)">(实习5折)</span>' : ''}</span>
  ...
  function fireEmployee(id) {
    ...
    if (!confirm('确定解雇 ' + emp.name + '？需支付 ' + comp.toFixed(1) + ' 万赔偿金')) return;
    G.money -= comp * (typeof WAN !== 'undefined' ? WAN : 10000);
  ...
  function renderNPCPanel(optEl) {
    ...
    let html = '';
    if (typeof NPCS === 'undefined') { el.innerHTML = '<div style="font-size:11px;color:var(--text-muted);padding:8px 0">NPC数据未加载</div>'; return; }
  ...
  function renderQuestPanel(optEl) {
    ...
    let html = '<div style="font-size:13px;font-weight:700;color:var(--accent);margin-bottom:10px;">📋 NPC任务线</div>';
    let hasQuests = false;
  ...
  function renderActDisplay() {
    ...
    if (typeof MILESTONES !== 'undefined' && ms < MILESTONES.length) {
      ...
      const okCol = allOk ? 'var(--green)' : 'var(--text-muted)';
      ...
      nextMsHtml = `
        <div style="margin-top:10px;padding:8px 10px;background:rgba(255,215,0,0.05);border-radius:8px;border:1px solid rgba(255,215,0,0.15)">
          ...
          <div style="font-size:10px;color:var(--text-secondary);line-height:1.6">
            ${moneyOk ? '✅' : '⏳'} 资产 ≥ ${(m.money / (typeof WAN !== 'undefined' ? WAN : 10000)).toFixed(0)}万<br>
    ...
    el.innerHTML = `
      <div style="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:8px">${actNames[act] || `第${act}幕`}</div>
      <div class="stat-row" style="font-size:11px"><span class="stat-label">里程碑</span><span class="stat-value">${ms}/5</span></div>
      <div class="stat-bar"><div class="stat-bar-fill" style="width:${(ms/5)*100}%;background:var(--accent)"></div></div>
      ${nextMsHtml}
  ...
  function renderHotSearch() {
    ...
    el.innerHTML = '<div style="max-height:340px;overflow-y:auto;">' + newsList.slice(0, 10).map((h, i) => {
      ...
      const catColor = isPos ? 'var(--green)' : 'var(--red)';
      const catLabel = h.category || '';
      return `<div class="hot-item" style="cursor:pointer;padding:4px 0;display:flex;align-items:center;gap:8px;" onclick="UI.showNewsDetail('${h.id || ''}')" title="${escapeHtml(h.text)}">
        <span class="hot-rank ${i < 3 ? 'top3' : ''}" style="flex-shrink:0;">${i + 1}</span>
        <span style="font-size:11px;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(h.text)}</span>
        ${catLabel ? `<span style="font-size:9px;color:${catColor};margin-left:auto;flex-shrink:0;">${catLabel}</span>` : ''}
      </div>`;
    }).join('') + '</div>';
  }
  ...
  // ========== 事件日志 ==========
  let eventLogSearch = '';
  let _lastEventLogCount = 0;
  let _lastEventLogSearch = '';
  ...
  function renderEventLog() {
    ...
    var totalLogs = G.eventLog.length;
    var logCap = 200;

    // 搜索模式或搜索词变更 → 全量重建
    if (searchTerm || _lastEventLogSearch !== searchTerm) {
      _lastEventLogCount = totalLogs;
      _lastEventLogSearch = searchTerm;
      var logs = G.eventLog.slice(0, logCap);
      if (searchTerm) {
      ...
      var html = '<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;">' +
        ...
        '<button style="font-size:10px;padding:2px 8px;background:var(--bg-hover);border:1px solid var(--border);border-radius:3px;color:var(--accent);cursor:pointer;white-space:nowrap;" onclick="UI.triggerBalanceCheck()" title="AI数值平衡自检">⚖ 自检</button>' +
        '</div>';
    ...
    // 非搜索模式 → 增量更新：仅插入新条目
    var newCount = totalLogs - _lastEventLogCount;
    if (newCount <= 0) return;
    _lastEventLogCount = totalLogs;

    // 首次渲染 log-entry 容器
    var scrollTop = el.scrollTop;
    var hasSearchBar = el.querySelector('#event-log-search');
    if (!hasSearchBar) {
      // 首次调用，初始化搜索栏 + 日志容器
      var toolbarHtml = '<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;">' +
        '<input type="text" id="event-log-search" placeholder="搜索日志..." value="" ' +
        'style="flex:1;font-size:11px;padding:4px 8px;background:var(--bg-primary);border:1px solid var(--border);border-radius:4px;color:var(--text-primary);outline:none;" ' +
        'oninput="UI._eventLogSearch(this.value)" />' +
        '<button style="font-size:10px;padding:2px 8px;background:var(--bg-hover);border:1px solid var(--border);border-radius:3px;color:var(--accent);cursor:pointer;white-space:nowrap;" onclick="UI.triggerBalanceCheck()" title="AI数值平衡自检">⚖ 自检</button>' +
    ...
    var entriesEl = el.querySelector('#log-entries');
    if (!entriesEl) return;

    var newLogs = G.eventLog.slice(0, newCount);
    var fragHtml = '';
    for (var i = newLogs.length - 1; i >= 0; i--) {
      fragHtml += '<div class="log-entry">' +
        '<span class="log-time">[' + newLogs[i].time + ']</span>' +
        '<span class="log-text">' + escapeHtml(newLogs[i].text) + '</span>' +
        '</div>';
    }
    var oldScrollHeight = el.scrollHeight;
    entriesEl.insertAdjacentHTML('afterbegin', fragHtml);
    el.scrollTop = scrollTop + (el.scrollHeight - oldScrollHeight);

    // 超过上限移除末尾
    var entries = entriesEl.children;
    while (entries.length > logCap && entries.length > 0) {
      entriesEl.removeChild(entries[entries.length - 1]);
    }
  }
  ...
  function highlightText(text, query) {
    ...
    return escaped.replace(regex, '<mark style="background:rgba(245,158,11,0.3);color:var(--accent);padding:0 1px;border-radius:1px;">$1</mark>');
  }
  ...
  function openAchievementPanel() {
    ...
    const total = (typeof ACHIEVEMENTS !== 'undefined' ? ACHIEVEMENTS : []).length;
    const unlocked = (G.unlockedAchievements || []).length;
    const pct = total > 0 ? Math.round(unlocked / total * 100) : 0;

    let html = `<div style="margin-bottom:12px"><div style="font-size:12px;color:var(--text-secondary)">进度：${unlocked}/${total}（${pct}%）</div><div class="stat-bar" style="margin-top:6px"><div class="stat-bar-fill" style="width:${pct}%;background:var(--accent)"></div></div></div>`;
    ...
    ACHIEVEMENTS.forEach(a => {
      ...
      html += `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);opacity:${done?1:0.35}">
        ...
        <div style="flex:1">
          ...
          ${done ? `<div style="font-size:10px;color:var(--accent);margin-top:2px">🏅 ${getAchievementRewardDesc(a.id) || '永久加成已生效'}</div>` : ''}
        </div>
        ${done && !read ? '<div style="width:8px;height:8px;border-radius:50%;background:var(--accent);flex-shrink:0" title="新解锁"></div>' : ''}
      </div>`;
    });
  ...
  function renderAchievementPanel(panel) {
    ...
    let html = '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">';
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent)">🏆 成就殿堂</div>';
    html += '</div>';
    
    html += `<div style="margin-bottom:12px"><div style="font-size:12px;color:var(--text-secondary)">进度：${unlocked}/${total}（${pct}%）| 累积加成：收入+${((achRewards.incomeMult||0)*100).toFixed(0)}%</div><div class="stat-bar" style="margin-top:6px"><div class="stat-bar-fill" style="width:${pct}%;background:var(--accent)"></div></div></div>`;
    ...
    ACHIEVEMENTS.forEach(a => {
      ...
      html += `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);opacity:${done?1:0.35}">
        ...
        <div style="flex:1">
          ...
          ${done ? `<div style="font-size:9px;color:var(--accent);margin-top:1px">🏅 ${getAchievementRewardDesc(a.id) || '永久加成'}</div>` : ''}
        </div>
      </div>`;
    });
    panel.innerHTML = html;
  ...
  function showCityOverview() {
    ...
    cityIds.forEach(function(cid) {
      ...
      html += '<div style="margin-bottom:12px;padding:10px;background:var(--bg-hover);border-radius:8px;border-left:3px solid ' + (isCurrent ? 'var(--accent)' : (unlocked ? 'var(--green)' : 'var(--border)')) + '">';
      ...
      if (isCurrent) html += '<span style="font-size:9px;padding:1px 6px;background:rgba(245,158,11,0.15);color:var(--accent);border-radius:4px;">当前</span>';
      if (!unlocked) {
      ...
      html += '<div style="font-size:10px;color:var(--accent);margin-bottom:4px;">🏆 ' + (city.cityBonus ? city.cityBonus.desc : '无特殊加成') + '</div>';
      if (unlocked) {
  ...
  function renderAutoButton() {
    try {
      ...
      SGame.DEBUG && console.error('[商海浮沉] renderAutoButton error:', e);
    }
  }
  ...
  function toggleAutoMode() {
    try {
      ...
      SGame.DEBUG && console.error('[商海浮沉] toggleAutoMode error:', e);
      try { renderAutoButton(); } catch(_) { SGame.DEBUG && console.error('[ui] renderAutoButton failed:', _ && _.message); }
    }
  }
  ...
  function renderBalanceReport(content, G, data, result, fm, hasLLM, llmError) {
    ...
    if (hasLLM && result && (result.summary || (result.suggestions && result.suggestions.length > 0))) {
      ...
      if (result.suggestions && result.suggestions.length > 0) {
        ...
        result.suggestions.forEach(function(s, i) {
          ...
          html += '<td style="color:var(--accent)">' + escapeHtml(String(s.suggested || '---')) + '</td>';
    ...
    if (data.bizROI && data.bizROI.length > 0) {
      ...
      data.bizROI.forEach(function(b, i) {
        ...
        html += '<td style="color:' + (b.roi > 1.5 ? 'var(--green)' : (b.roi > 0.8 ? 'var(--text-secondary)' : 'var(--red)')) + '">' + b.roi + '</td>';
  ...
  function renderManualButton() {
    ...
    container.innerHTML = `
      <button id="btn-manual" class="btn btn-action ${onCd ? 'cooldown' : ''}" 
        style="width:100%; padding:12px 20px; font-size:15px;"
        ${onCd ? 'disabled' : ''} 
        onclick="UI.doManualWork()">
        ${onCd ? `冷却 ${cdRemain}s` : '拉项目 / 谈合作 / 推广赞助'}
      </button>
    `;
  }
  ...
  function startCdTimer() {
    ...
    cdTimer = setInterval(() => {
      ...
      if (remain <= 0) {
        ...
        btn.disabled = false;
        btn.textContent = '拉项目 / 谈合作 / 推广赞助';
        clearInterval(cdTimer);
        cdTimer = null;
      } else {
        btn.textContent = `冷却 ${remain}s`;
      }
    }, 1000);
  }
  ...
  function renderSkillTreeContent(container) {
    ...
    let html = `<div style="margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;">
      <span style="font-size:13px;color:var(--accent);">可用技能点: <b>${G.statPoints || 0}</b></span>
      <span style="font-size:11px;color:var(--text-muted);">已解锁: ${(G.unlockedSkills || []).length}/20</span>
      ${costReduction > 0 ? `<span style="font-size:10px;color:var(--accent);">🏅 成就减免: -${costReduction}点</span>` : ''}
    </div>`;

    const catNames = { management:'管理', tech:'技术', social:'社交', finance:'金融' };
    Object.entries(SKILL_TREES).forEach(([cat, skills]) => {
      html += `<div style="margin-bottom:14px;">
        <div style="font-size:13px;font-weight:600;color:var(--accent);margin-bottom:8px;border-bottom:1px solid var(--border);padding-bottom:4px;">${catNames[cat] || cat}</div>`;
      skills.forEach(sk => {
        ...
        const canUnlock = !unlocked && !isBlocked && (G.statPoints || 0) >= effectiveCost && (typeof SGame.canUnlockSkill === 'function' ? SGame.canUnlockSkill(sk) : true);
        
        html += `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);opacity:${unlocked ? 0.6 : (isBlocked ? 0.4 : 1)};position:relative;${sk.exclusive ? 'border-left:3px solid var(--accent);padding-left:7px;' : ''}">
          <div style="flex:1;">
            <div style="font-size:12px;font-weight:600;${unlocked?'color:var(--accent);':(isBlocked?'color:var(--text-muted);':'')}">${sk.name} ${unlocked?'✓':''} ${sk.exclusive ? '<span style="font-size:9px;color:var(--accent);background:rgba(245,158,11,0.12);padding:1px 5px;border-radius:3px;">⚡互斥</span>' : ''}</div>
            <div style="font-size:10px;color:var(--text-muted);margin-top:2px;">${sk.desc || '无描述'}</div>
            ${isBlocked ? `<div style="font-size:9px;color:var(--red);margin-top:2px;">🚫 已被「${SKILL_TREES[sk.exclusive].find(s=>s.id===exclusive.lockedBy)?.name||exclusive.lockedBy}」锁定</div>` : ''}
          </div>
          ${canUnlock ? `<button class="btn" style="font-size:10px;padding:3px 8px;" onclick="UI.buySkill('${sk.id}')">升级 (${effectiveCost}点)</button>` : (unlocked ? '<span style="font-size:10px;color:var(--green);">已解锁</span>' : (isBlocked ? '<span style="font-size:10px;color:var(--text-muted);">不可用</span>' : `<span style="font-size:10px;color:var(--text-muted);">需${effectiveCost}点</span>`))}
        </div>`;
      });
      html += '</div>';
    });
  ...
  function buySkill(skillId) {
    if (typeof SGame.unlockSkill === 'function') {
      ...
      if (ok) {
        ...
        // 升级失败时刷新技能树（确保按钮状态正确）并给出提示
        const content = document.getElementById('skills-content');
        if (content) renderSkillTreeContent(content);
        renderAll();
        let reason = '升级失败，请检查是否满足所有前置条件';
        if (typeof SGame.whyCannotUnlock === 'function') {
          const detail = SGame.whyCannotUnlock(skillId);
          if (detail) reason = detail;
        }
        showToast('⚠️', '技能升级', reason, 3000);
      }
    }
  }
  ...
  function showBankruptcyPanel() {
    ...
    overlay.innerHTML = `
        <div>
          ...
          <h1 style="font-size:32px;margin-bottom:12px;background:linear-gradient(135deg,var(--red),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent">${ending.title}</h1>
  ...
  function renderStatPanel(container) {
    ...
    if (G.employees) {
      G.employees.forEach(function(emp) {
        ...
        salaryPerTick += actual * (typeof WAN !== 'undefined' ? WAN : 10000);
      });
    }
    ...
    // 股票市值
    var stockValue = 0;
    try { if (typeof SGame.getStockPortfolioValue === 'function') stockValue = SGame.getStockPortfolioValue(); } catch(e) { SGame.DEBUG && console.warn('[UI] getStockPortfolioValue failed:', e.message || e); }
    ...
    try { if (typeof SGame.getSubsidiarySummary === 'function') subSummary = SGame.getSubsidiarySummary(); } catch(e) { SGame.DEBUG && console.warn('[UI] getSubsidiarySummary failed:', e.message || e); }
    ...
    if (typeof SGame.calcBizIncome === 'function' && typeof BUSINESS_DEFS !== 'undefined') {
    ...
    if (typeof NPCS !== 'undefined') Object.entries(G.npcFavor || {}).forEach(function(entry) {
    ...
    if (ast && ast.totalTicks > 0) {
      ...
      autoHtml += renderAutoDashboard();
    }

    // 标题栏
    var html = '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">';
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent)">📈 经营统计</div>';
    html += '</div>';

    html += section('财务概览');
    html += row('当前资产', SGame.formatMoney(curMoney), 'var(--accent)');
    html += row('累计总收入', SGame.formatMoney(totalIncome), 'var(--green)');
    html += row('累计总支出', SGame.formatMoney(totalExpense), 'var(--red)');
    html += row('净收入', SGame.formatMoney(netIncome), netIncome >= 0 ? 'var(--green)' : 'var(--red)');
    html += row('每Tick收入', SGame.formatMoney(perTickIncome), 'var(--green)');
    html += row('每Tick支出', SGame.formatMoney(perTickExpense), 'var(--red)');
    html += row('每Tick工资支出', SGame.formatMoney(salaryPerTick), 'var(--red)');
    html += row('每Tick维护成本', SGame.formatMoney(maintenanceCost), 'var(--red)');
    html += row('股票持仓市值', SGame.formatMoney(stockValue));
    html += row('资产总值', SGame.formatMoney(assetValue));
    html += row('贷款总额', SGame.formatMoney(loanTotal), 'var(--red)');
    html += row('子公司累计收益', SGame.formatMoney(subSummary.totalIncome), 'var(--green)');
  ...
  function renderAutoDashboard() {
    ...
    var runtime = SGame.getStrategyRuntime ? SGame.getStrategyRuntime() : {};
    var heap = SGame.getStrategyHeap ? SGame.getStrategyHeap() : [];
    var now = G.tickCount;
    var am = G.autoMode;
    am.cooldowns = am.cooldowns || {};

    var html = '<div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border)">';
    html += '<div style="font-size:12px;font-weight:600;color:var(--accent);margin-bottom:8px">📋 策略执行详情</div>';

    // Strategy list from core.js STRATEGIES (mirror the 15 strategies)
    var strategies = [
      { key:'eventDecide', name:'自动决策' },
      { key:'repay', name:'还款' },
      { key:'unlock', name:'区域解锁' },
      { key:'openBiz', name:'业务开设' },
      { key:'upgrade', name:'业务升级' },
      { key:'hire', name:'招聘' },
      { key:'fire', name:'解雇' },
      { key:'research', name:'研发' },
      { key:'invest', name:'股票' },
      { key:'gift', name:'送礼' },
      { key:'manualWork', name:'拉项目' },
      { key:'negotiate', name:'商务约谈' },
      { key:'assetBuy', name:'资产投资' },
      { key:'assetPawn', name:'资产典当' },
      { key:'rest', name:'员工休息' },
    ];

    for (var i = 0; i < strategies.length; i++) {
      var s = strategies[i];
      if (am[s.key] === false) continue; // 用户禁用的策略不显示

      var rt = runtime[s.key];
      var lastTick = rt ? rt.tick : 0;
      var lastTime = rt && rt.time ? new Date(rt.time) : null;
      var cdRemain = 0;
      // 从堆中查找冷却剩余
      for (var j = 0; j < heap.length; j++) {
        if (heap[j].key === s.key) {
          cdRemain = Math.max(0, heap[j].nextTick - now);
          break;
        }
      }
      ...
      var statusText = '未执行';
      if (rt) {
        statusColor = rt.result === 'ok' ? 'var(--green)' : 'var(--red)';
        statusText = '最近: Tick' + lastTick;
        if (lastTime) {
          var hh = lastTime.getHours().toString().padStart(2, '0');
          var mm = lastTime.getMinutes().toString().padStart(2, '0');
          statusText += ' ' + hh + ':' + mm;
        }
      }
      var cdText = cdRemain > 0 ? ' | CD:' + cdRemain + 't' : '';

      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0;font-size:10px;border-bottom:1px solid rgba(255,255,255,0.04)">';
      html += '<span style="color:var(--text-secondary)">' + s.name + '</span>';
      html += '<span style="color:' + statusColor + '">' + statusText + cdText + '</span>';
      html += '</div>';
    }
    html += '</div>';
    return html;
  ...
  function renderMilestonePanel(container) {
    ...
    let html = '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">';
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent)">🏅 里程碑</div>';
    html += '</div>';
    ...
    coreMilestones.forEach(function(m, i) {
      ...
      const bizOk = bizSum >= m.bizSumMin;
      const allOk = moneyOk && repOk && bizOk;
      const tierColors = ['var(--text-muted)', '#c0c8d4', 'var(--green)', 'var(--accent)', 'var(--purple)', 'var(--accent)'];
      ...
      html += '<div style="font-size:12px;font-weight:600;color:' + (done ? tierColors[tierIdx] : (allOk ? 'var(--green)' : 'var(--text-muted)')) + '">' + (i + 1) + '. ' + m.name + (done ? ' ✓' : '') + '</div>';
      html += '<div style="font-size:10px;color:var(--text-secondary);margin-top:3px;line-height:1.5">';
      html += (done || moneyOk ? '✅' : '⏳') + ' 资产≥' + (m.money / WAN).toFixed(0) + '万 | ';
    ...
    html += '<div class="stat-bar" style="margin-bottom:12px;"><div class="stat-bar-fill" style="width:' + (advancedTotal > 0 ? (advancedAchieved / advancedTotal * 100) : 0) + '%;background:var(--accent)"></div></div>';
    ...
    advMilestones.forEach(ms => {
      ...
      if (done) html += '<div style="font-size:9px;color:var(--accent);margin-top:2px">✓ 已达成 | +2技能点</div>';
      html += '</div></div>';
    });
  ...
  function renderRankingPanel(container) {
    ...
    let html = '<div style="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:4px;">🏆 商界排行榜</div>';
    html += '<div style="font-size:11px;color:var(--text-muted);margin-bottom:14px;">你的排名: <b style="color:var(--accent);font-size:14px;">第 ' + rankInfo.rank + ' 名</b> / 共 ' + rankInfo.total + ' 位</div>';
    html += '<div style="display:flex;flex-direction:column;gap:6px;">';
    rankInfo.list.forEach((entity, i) => {
      const isPlayer = entity.isPlayer;
      const bg = isPlayer ? 'background:rgba(0,210,255,0.1);border:1px solid var(--accent);' : 'border:1px solid var(--border);';
      const nameColor = isPlayer ? 'var(--accent)' : 'var(--text-primary)';
      ...
      html += '<div style="padding:10px 14px;border-radius:8px;' + bg + 'display:flex;align-items:center;gap:12px;">' +
        ...
        '<span style="font-size:12px;color:var(--accent);min-width:100px;text-align:right;font-weight:600;">' + SGame.formatMoney(entity.money) + '</span>' +
  ...
  function showNewsDetail(newsId) {
    ...
    const catColor = isPos ? 'var(--green)' : 'var(--red)';
    const emoji = isPos ? '📈' : '📉';
    showToast(emoji, `[${escapeHtml(item.category)}] ${escapeHtml(item.text)}`, '', 3000);
  }
  ...
  function renderTechPanel(container) {
    ...
    let html = '<div style="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:12px;">🔬 科技研发</div>' +
      '<div style="font-size:12px;color:var(--text-secondary);margin-bottom:14px;">研发点数 (RPT): <b style="color:var(--accent)">' + (G.rpt||0) + '</b></div>';
    ...
    routes.forEach(route => {
      ...
      html += '<div style="margin-bottom:16px;border:1px solid var(--border);border-radius:8px;padding:12px;">' +
        ...
        '<div class="stat-bar" style="margin-bottom:10px;"><div class="stat-bar-fill" style="width:' + (completed/levels.length*100) + '%;background:var(--accent)"></div></div>';
      ...
      levels.forEach((tech, i) => {
        ...
        if (isComplete) { statusColor = 'var(--green)'; statusText = '✓ 已完成'; }
        else if (isActive) { statusColor = 'var(--accent)'; statusText = '研发中... 剩余 ' + active.remainingTicks + ' Tick'; }
        else if (isLocked) { statusColor = 'var(--text-muted)'; statusText = '🔒 第' + tech.unlockAct + '幕解锁'; }
        else if (canStart) { statusColor = 'var(--accent)'; statusText = '可研发'; }
  ...
  function renderStockPanel(container) {
    ...
    let html = '<div style="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:12px;">📈 股票投资</div>';
    ...
    html += '<div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">' +
      '投资组合市值: <b id="stock-portfolio-val" style="color:var(--accent)">' + SGame.formatMoney(portfolioVal) + '</b> | ' +
      '成本: ' + SGame.formatMoney(costBasis) + ' | ' +
      '盈亏: <b id="stock-pnl" style="color:' + pnlColor + '">' + pnlSign + SGame.formatMoney(pnl) + '</b>' +
      '</div>';
    ...
    Object.entries(STOCKS).forEach(([id, stock]) => {
      ...
      html += '<div id="stock-' + id + '" style="padding:10px;border:1px solid var(--border);border-radius:6px;font-size:11px;">' +
        '<div style="font-weight:600;">' + stock.name + ' <span style="font-size:10px;color:var(--text-muted)">' + stock.sector + '</span></div>' +
        '<div style="color:var(--accent);margin:4px 0;">¥<span id="stock-price-' + id + '">' + price.toFixed(2) + '</span> <span id="stock-change-' + id + '" style="color:' + color + ';">' + sign + changePct + '%</span></div>' +
        (holding > 0 ? '<div style="color:var(--text-muted);font-size:10px;">持仓: ' + holding + '股</div>' : '') +
        '<div style="display:flex;gap:4px;margin-top:6px;">' +
        '<button class="btn" style="font-size:10px;padding:2px 8px;" onclick="SGame.buyStock(\'' + id + '\',10);UI._stockNeedsFullRefresh=true;UI.renderAll();">买10</button>' +
        '<button class="btn" style="font-size:10px;padding:2px 8px;" onclick="SGame.buyStock(\'' + id + '\',100);UI._stockNeedsFullRefresh=true;UI.renderAll();">买100</button>' +
        (holding > 0 ? '<button class="btn" style="font-size:10px;padding:2px 8px;background:var(--red);" onclick="SGame.sellStock(\'' + id + '\',' + Math.min(holding,10) + ');UI._stockNeedsFullRefresh=true;UI.renderAll();">卖10</button>' : '') +
        '</div></div>';
    });
    html += '</div>';

    // 贷款
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent);margin-bottom:12px;margin-top:20px;border-top:1px solid var(--border);padding-top:16px;">🏦 银行贷款</div>';

    const maxLoan = Math.floor(G.money * 0.5);
    const rep = G.reputation || 0;
    const npcBon = (typeof SGame.calcNpcBonus === 'function') ? SGame.calcNpcBonus() : {};
    ...
    html += '<div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">' +
      '可贷额度: <b style="color:var(--accent)">' + SGame.formatMoney(maxLoanActual) + '</b> (资产50%' + (npcBon._loanCapBonus ? '+金行长加成' : '') + ') | ' +
      '利率: <b>' + rateDisp + '%</b></div>';
    ...
    if (G.loans && G.loans.length > 0) {
      ...
      G.loans.forEach((loan, i) => {
        html += '<div style="padding:8px;border:1px solid var(--border);border-radius:6px;margin-bottom:6px;font-size:11px;display:flex;justify-content:space-between;align-items:center;">' +
          ...
          '<button class="btn" style="font-size:10px;padding:3px 8px;" onclick="SGame.repayLoan(' + loan.id + ');UI._stockNeedsFullRefresh=true;UI.renderAll();">还款</button>' +
        '</div>';
      });
    }
    ...
    if (canLoan && maxLoan > 10000) {
      ...
      [Math.floor(maxLoan*0.2), Math.floor(maxLoan*0.4), Math.floor(maxLoan*0.6)].forEach(amt => {
        if (amt < 10000) return;
        html += '<button class="btn" style="font-size:10px;padding:4px 10px;margin:4px;" onclick="SGame.applyLoan(' + amt + ',60);UI._stockNeedsFullRefresh=true;UI.renderAll();">贷 ' + SGame.formatMoney(amt) + ' (60Tick)</button>';
      });
    } else if (!canLoan) {
      html += '<div style="font-size:11px;color:var(--red);margin-top:10px;">已达贷款上限（最多3笔）</div>';
    }

    container.innerHTML = html;
  }

  // 增量更新股票价格：仅更新DOM中已有股价/涨跌幅的textContent
  function _updateStockPrices() {
    var G = SGame.G;
    if (!G || !G.stockPrices) return;
    // 更新投资组合摘要
    var portfolioEl = document.getElementById('stock-portfolio-val');
    var pnlEl = document.getElementById('stock-pnl');
    if (portfolioEl) {
      var portfolioVal = typeof SGame.getStockPortfolioValue === 'function' ? SGame.getStockPortfolioValue() : 0;
      var costBasis = typeof SGame.getStockCostBasis === 'function' ? SGame.getStockCostBasis() : 0;
      portfolioEl.textContent = SGame.formatMoney(portfolioVal);
      if (pnlEl) {
        var pnl = portfolioVal - costBasis;
        var pnlSign = pnl >= 0 ? '+' : '';
        pnlEl.textContent = pnlSign + SGame.formatMoney(pnl);
    ...
    Object.entries(STOCKS).forEach(function(entry) {
      var id = entry[0];
      var stock = entry[1];
      var price = G.stockPrices ? (G.stockPrices[id] || stock.basePrice) : stock.basePrice;
      var priceEl = document.getElementById('stock-price-' + id);
      var changeEl = document.getElementById('stock-change-' + id);
      if (priceEl) {
        priceEl.textContent = price.toFixed(2);
      }
      if (changeEl) {
        var change = price - stock.basePrice;
        var changePct = (change / stock.basePrice * 100).toFixed(1);
        var sign2 = change >= 0 ? '+' : '';
        changeEl.textContent = sign2 + changePct + '%';
  ...
  function renderAssetPanel(panel) {
    ...
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent);">💎 资产中心</div>';
    ...
    html += '<div style="font-size:13px;font-weight:700;color:var(--accent);margin-bottom:8px;">📈 资产市场</div>';
    ...
    if (listings.length === 0) {
      ...
      for (var i = 0; i < listings.length; i++) {
        ...
        var rarityColor = { common:'var(--text-secondary)', uncommon:'var(--accent-green)', rare:'var(--accent)', epic:'var(--purple)' }[l.rarity] || 'var(--text-secondary)';
        ...
        html += '<div style="font-size:14px;font-weight:700;color:var(--accent);">' + (typeof SGame.formatMoney === 'function' ? SGame.formatMoney(l.price) : l.price) + '</div>';
        html += '<button onclick="SGame.buyAsset(' + i + ');UI.renderAssetPanel(document.getElementById(\'center-panel\'));UI.renderAll();" ' +
    ...
    // 右侧：我的资产
    html += '<div style="flex:1;min-width:320px;">';
    html += '<div style="font-size:13px;font-weight:700;color:var(--accent);margin-bottom:8px;">🏠 我的资产 (' + G.assets.length + '/' + ((typeof CONFIG !== 'undefined' && CONFIG.ASSET_MAX_SLOTS) || 20) + ')</div>';
    if (G.assets.length === 0) {
      ...
      for (var j = 0; j < G.assets.length; j++) {
        ...
        var aRarityColor = { common:'var(--text-secondary)', uncommon:'var(--accent-green)', rare:'var(--accent)', epic:'var(--purple)' }[a.rarity] || 'var(--text-secondary)';
        var curVal = a.currentPrice || a.purchasePrice;
        var pnl = curVal - a.purchasePrice;
        var pnlColor = pnl >= 0 ? 'var(--red)' : 'var(--green)';
        var pnlSign = pnl >= 0 ? '+' : '';
        var inAuction = G.assetAuctionList && G.assetAuctionList.some(function(auc) { return auc.assetId === a.id; });

        html += '<div class="owned-asset-card" style="border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:8px;background:rgba(255,255,255,0.02);' + (inAuction ? 'border-color:var(--accent);' : '') + '">';
        html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
        html += '<div>';
        ...
        if (inAuction) html += '<span style="font-size:10px;color:var(--accent);margin-left:6px;">🔨 拍卖中</span>';
        html += '</div>';
        html += '<div style="text-align:right;">';
        html += '<div style="font-size:14px;font-weight:700;color:var(--accent);">' + (typeof SGame.formatMoney === 'function' ? SGame.formatMoney(curVal) : curVal) + '</div>';
        ...
        if (!inAuction) {
          ...
          // 典当按钮
          html += '<button onclick="if(confirm(\'⚠️ 典当将以市场价的38%-55%紧急变现，确认？\')){SGame.pawnAsset(\'' + a.id + '\');UI.renderAssetPanel(document.getElementById(\'center-panel\'));UI.renderAll();}" ' +
          ...
          if (aucIdx >= 0) {
            ...
            html += '<button onclick="SGame.cancelAuction(\'' + auc.assetId + '\');UI.renderAssetPanel(document.getElementById(\'center-panel\'));UI.renderAll();" ' +
    ...
    if (G.assetAuctionList && G.assetAuctionList.length > 0) {
      ...
      html += '<div style="font-size:13px;font-weight:700;color:var(--accent);margin-bottom:8px;">🔨 拍卖行（进行中）</div>';
      ...
      for (var ak = 0; ak < G.assetAuctionList.length; ak++) {
        ...
        html += '<div style="border:1px solid var(--accent);border-radius:8px;padding:10px;min-width:180px;background:rgba(6,182,212,0.05);">';
        ...
        html += '<div style="font-size:10px;color:' + (remaining <= 0 ? 'var(--green)' : 'var(--text-secondary)') + ';">' + (remaining <= 0 ? '即将成交' : '剩余 ' + remaining + ' Tick') + '</div>';
        html += '</div>';
      }
      html += '</div></div>';
    }
  ...
  function renderNewsFeed() {
    ...
    if (existingNews) {
      // 增量更新：复用已有 DOM 节点，仅更新文本内容
      var textEl = existingNews.querySelector('.log-text');
      if (textEl) textEl.textContent = latest.text;
      var timeEl = existingNews.querySelector('.log-time');
      if (timeEl) timeEl.textContent = 'Tick' + latest.tick;
      return;
    }
    ...
    newsDiv.style.cssText = 'border-left: 2px solid var(--accent); padding-left: 8px; background: rgba(6,182,212,0.05); border-radius: 4px;';
    newsDiv.innerHTML = '<span class="log-time">Tick' + latest.tick + '</span> <span style="font-size:10px;color:var(--accent)">📰 商业快讯</span><br><span class="log-text">' + escapeHtml(latest.text) + '</span>';
    if (logEl.firstChild) {
  ...
  function renderIntelPanel(panel) {
    ...
    let html = '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">';
    html += _renderBackButton('dashboard');
    html += '<div style="font-size:16px;font-weight:700;color:var(--accent)">🔍 商业情报</div>';
    html += '</div>';
  ...
  // ========== 年度财报面板 ==========
  function renderFinancialPanel(panel) {
    if (!panel) return;
    var G = SGame.G;
    if (!G) return;
    var reports = G.financialReports || [];
    ...
    html += '<div style="font-size:18px;font-weight:700;color:var(--accent);margin-bottom:12px;border-bottom:1px solid var(--border);padding-bottom:8px;">📊 年度财报</div>';

    if (reports.length === 0) {
      html += '<div style="text-align:center;padding:40px 20px;color:var(--text-muted);">';
      html += '<div style="font-size:48px;margin-bottom:12px;">📊</div>';
      html += '<div style="font-size:14px;">尚未生成任何年度财报</div>';
      html += '<div style="font-size:12px;color:var(--text-secondary);margin-top:4px;">每365个tick（约一个游戏年）自动生成一份</div>';
      if (G._annualCheckpoint) {
        var tickUntil = 365 - (G.tickCount - G._annualCheckpoint.tick);
        html += '<div style="font-size:11px;color:var(--accent);margin-top:8px;">距离下一份财报还需 ' + Math.max(0, tickUntil) + ' tick</div>';
      }
      html += '</div>';
      panel.innerHTML = html;
      return;
    ...
    // 最新财报摘要卡片
    var latest = reports[reports.length - 1];
    var netClass = latest.netProfit >= 0 ? 'color:#22c55e;' : 'color:#ef4444;';
    var netSign = latest.netProfit >= 0 ? '+' : '';
    html += '<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:12px;">';
    html += '<div style="font-size:14px;font-weight:700;color:var(--accent);margin-bottom:8px;">最新财报 #' + latest.id + '</div>';
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 16px;font-size:12px;">';
    html += '<div style="color:var(--text-secondary);">周期</div><div>Tick ' + latest.startTick + ' - ' + latest.endTick + '</div>';
    html += '<div style="color:var(--text-secondary);">总收入</div><div style="color:#22c55e;">' + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(latest.totalRevenue) : latest.totalRevenue.toLocaleString()) + ' W</div>';
    html += '<div style="color:var(--text-secondary);">总支出</div><div style="color:#ef4444;">' + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(latest.totalExpenses) : latest.totalExpenses.toLocaleString()) + ' W</div>';
    html += '<div style="color:var(--text-secondary);">净利润</div><div style="' + netClass + 'font-weight:700;">' + netSign + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(latest.netProfit) : latest.netProfit.toLocaleString()) + ' W</div>';
    html += '<div style="color:var(--text-secondary);">员工数</div><div>' + latest.startEmployees + ' → ' + latest.endEmployees + (latest.endEmployees > latest.startEmployees ? ' <span style="color:#22c55e;">+' + (latest.endEmployees - latest.startEmployees) + '</span>' : (latest.endEmployees < latest.startEmployees ? ' <span style="color:#ef4444;">' + (latest.endEmployees - latest.startEmployees) + '</span>' : '')) + '</div>';
    html += '<div style="color:var(--text-secondary);">业务数</div><div>' + latest.startBusinesses + ' → ' + latest.endBusinesses + (latest.endBusinesses > latest.startBusinesses ? ' <span style="color:#22c55e;">+' + (latest.endBusinesses - latest.startBusinesses) + '</span>' : '') + '</div>';
    html += '<div style="color:var(--text-secondary);">资产数</div><div>' + (latest.totalAssets || 0) + '</div>';
    html += '<div style="color:var(--text-secondary);">股票市值</div><div>' + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(latest.stockValue || 0) : (latest.stockValue || 0).toLocaleString()) + ' W</div>';
    html += '<div style="color:var(--text-secondary);">声誉</div><div>' + (latest.reputation || 0) + '</div>';
    html += '<div style="color:var(--text-secondary);">经济状态</div><div>' + (latest.economicState || 'stable') + '</div>';
    html += '</div></div>';

    // 历史财报列表
    if (reports.length > 1) {
      html += '<div style="font-size:13px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;border-bottom:1px solid var(--border);padding-bottom:4px;">历史财报 (' + reports.length + ')</div>';
      html += '<div style="display:flex;flex-direction:column;gap:6px;">';
      for (var i = reports.length - 2; i >= 0; i--) {
        var r = reports[i];
        var nc = r.netProfit >= 0 ? '#22c55e' : '#ef4444';
        var ns = r.netProfit >= 0 ? '+' : '';
        html += '<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:6px;padding:8px 10px;font-size:11px;display:flex;justify-content:space-between;align-items:center;">';
        html += '<div><span style="font-weight:600;">#' + r.id + '</span> <span style="color:var(--text-muted);">Tick ' + r.startTick + '-' + r.endTick + '</span></div>';
        html += '<div style="display:flex;gap:12px;">';
        html += '<span>营收: ' + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(r.totalRevenue) : r.totalRevenue.toLocaleString()) + 'W</span>';
        html += '<span style="color:' + nc + ';">净利: ' + ns + (typeof SGame !== 'undefined' && SGame.formatMoney ? SGame.formatMoney(r.netProfit) : r.netProfit.toLocaleString()) + 'W</span>';
        html += '<span>员工: ' + r.endEmployees + '</span>';
        html += '<span>业务: ' + r.endBusinesses + '</span>';
        html += '</div></div>';
      }
      html += '</div>';
    }
  ...
  function renderRivalReport() {
    ...
    if (existingReport) {
      var textEl = existingReport.querySelector('.log-text');
      if (textEl) textEl.textContent = latest.text;
      var timeEl = existingReport.querySelector('.log-time');
      if (timeEl) timeEl.textContent = 'Tick' + latest.tick;
      return;
    }
  ...
  function openAdvisor() {
    ...
    Advisor.getAdvice().then(function(result) {
      ...
      // 来源标签
      if (result.fromLLM) {
        html += '<div class="advisor-source llm">🤖 由 LLM 分析师生成</div>';
      } else {
        html += '<div class="advisor-source rule">📋 基于规则分析（LLM 不可用）</div>';
      }

      // 错误提示
      if (result.error && result.error !== 'busy') {
        html += '<div class="advisor-warning">⚠️ ' + escapeHtml(result.error) + '</div>';
      }
      if (result.error === 'busy') {
        html += '<div class="advisor-warning">⏳ 顾问正忙，请稍后再试。</div>';
      }

      // 建议内容（保留换行与简单 Markdown）
  ...
  // ========== 亮色/暗色/自动模式切换（三态循环：自动→亮色→暗色→自动） ==========
  function toggleTheme() {
    var mode = localStorage.getItem('shanghaifc-theme-mode');
    var theme = localStorage.getItem('shanghaifc-theme');
    var body = document.body;
    ...
    if (mode !== 'manual') {
      ...
      body.classList.remove('time-dawn', 'time-day', 'time-dusk', 'time-night');
      localStorage.setItem('shanghaifc-theme', 'light');
      localStorage.setItem('shanghaifc-theme-mode', 'manual');
    ...
    if (theme === 'light') {
      body.classList.remove('light-mode');
      localStorage.setItem('shanghaifc-theme', 'dark');
    ...
    // 状态3：手动暗色 → 返回自动模式
    body.classList.remove('light-mode');
    localStorage.removeItem('shanghaifc-theme-mode');
    localStorage.removeItem('shanghaifc-theme');
    if (btn) btn.textContent = '🌓';
  }
  ...
  return {
    ...
    renderAutoButton, toggleAutoMode, renderAutoDashboard,
    ...
    updateLLMQueueStatus,

    // ---- Topbar 事件委托 (data-action) ----
    _initTopbarDelegation: function() {
      var topbar = document.getElementById('topbar');
      if (!topbar || topbar._delegated) return;
      ...
      topbar.addEventListener('click', function(e) {
        ...
        var map = {
          ...
          bgm:          function() { if (typeof window.toggleBGM === 'function') window.toggleBGM(); },
          achievements: function() { openAchievementPanel(); },
          skills:       function() { openSkillTree(); },
          worldmap:     function() { switchPanel('worldmap'); },
          city:         function() { showCityOverview(); },
          milestone:    function() { switchPanel('milestone'); },
          intel:        function() { switchPanel('intel'); },
          advisor:      function() { openAdvisor(); },
          retire:       function() { retireGame(); },
        };
        if (map[act]) { map[act](); e.preventDefault(); }
      });
    },
  };
})();
