...
window.NPCSystem = (() => {
  ...
  function openDialog(npcId, dialogType) {
    ...
    if (typeof LLM !== 'undefined') {
      LLM.generateNPCDialog(npcId, dialogType, getFavorLevel(npcId)).then(text => {
        ...
        SGame.DEBUG && console.warn('[npc] LLM dialog failed:', e.message || e);
      });
    } else {
  ...
  function renderNPCActions(npcId) {
    ...
    if (npcId === 'zhaolei') {
      actions.push({ text: '询问行业趋势（人脉+1，好感+2）', fn: () => { changeFavor('zhaolei', 2); S.addConnections(1); EventSystem.addLog('赵磊分享了一些行业见解。'); } });
    ...
    SGame.DEBUG && console.log('[NPCSystem.renderNPCActions] npcId=' + npcId + ' actions count=' + actions.length);
    actions.forEach(function(a, i) { SGame.DEBUG && console.log('  action[' + i + ']: ' + a.text + ' idxInArray=' + actions.indexOf(a)); });
  ...
  function doAction(idx) {
    ...
    SGame.DEBUG && console.log('[NPCSystem.doAction] idx=' + idx + ' npcId=' + npcId + ' actionsLen=' + (currentActions ? currentActions.length : 'null'));
    if (currentActions && currentActions[idx]) {
      SGame.DEBUG && console.log('[NPCSystem.doAction] action[' + idx + '] text=' + currentActions[idx].text + ' hasFn=' + (typeof currentActions[idx].fn));
    }
    if (!npcId) { SGame.DEBUG && console.log('[NPCSystem.doAction] currentNPC is null, returning'); return; }

    // 优先使用闭包回调（支持动态索引，兼容任务线动态插入）
    if (currentActions[idx] && typeof currentActions[idx].fn === 'function') {
      SGame.DEBUG && console.log('[NPCSystem.doAction] executing fn for idx=' + idx);
      currentActions[idx].fn();
      return;
    }
    SGame.DEBUG && console.log('[NPCSystem.doAction] primary path failed, falling back');
